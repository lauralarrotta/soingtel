...
export function MonthlyStatsCards({
  ...
  const getColorClasses = (color: string, isActive: boolean) => {
    ...
    if (isActive) {
      return `${baseClasses} ring-2 ring-cyan-500 shadow-lg shadow-cyan-500/20`;
    }
    return `${baseClasses} hover:shadow-md hover:scale-[1.02]`;
  };
      ...
      {cards.map((card) => {
        ...
        return (
          <Card
            ...
            className={`cursor-pointer overflow-hidden ${getColorClasses(card.color, isActive)}`}
          >
